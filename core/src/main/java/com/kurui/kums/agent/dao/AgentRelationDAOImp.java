...
public class AgentRelationDAOImp extends BaseDAOSupport implements
	...
	public HttpServletRequest listAgentGroup(AgentRelationListForm alf,
		...
		try {
			...
			if (rootAgent != null) {
				...
				if (treeFileName == null || isExistFile == false) {
					treeFileName = DateUtil.getDateString("yyyyMMddHHmmss")
							+ ".xml";
					treeFilePath = Constant.SERVLET_XML_PATH + treeFileName;
	...
	public List list(AgentRelationListForm alf) throws AppException {
		Hql hql = new Hql();
		...
		if (Constant.toString(alf.getContactWay()) != "") {
			hql.add(" and (");
			hql.add(" ( ");
			hql.add(" a.rootAgent.name like '%" + alf.getContactWay().trim()
					+ "%'");
			...
			hql.add(" or ");
			hql.add(" ( ");
			hql.add(" a.relateAgent.name like '%" + alf.getContactWay().trim()
					+ "%'");
		...
		if (Constant.toLong(alf.getRelationType()) > 0) {
			hql.add(" and a.relationType=" + alf.getRelationType());
		}
	...
	public Document buildAgentTree(Agent rootAgent, String treeFileName,
			String treeFilePath, boolean includeRoot) throws AppException {
	...
	private Element setLevelAgentItem(Element rootItem, List<Agent> childList)
			throws AppException {
		...
		for (int i = 0; i < childList.size(); i++) {
			...
			if (tempRootAgent != null) {
				...
				List<Agent> tempChildList = getSubAgentList(tempRootAgent
						.getId());
	...
	/*
	 * public class TreeUtil { private Element rootItem; private
	 * ArrayList<Agent> rootAgentList; private ArrayList<Agent> subAgentList;
	 * private ArrayList<AgentGroup> agentGroupList; private int currentLevel;
	 * 
	 * public HttpServletRequest buildAgentTree(Agent rootAgent,
	 * ArrayList<ArrayList<Agent>> lists, HttpServletRequest request) { try {
	 * long agentId = rootAgent.getId(); rootAgentList = new ArrayList<Agent>();
	 * rootAgentList.add(rootAgent);
	 * 
	 * rootItem = new Element("tree"); rootItem.setAttribute("id", "0");
	 * Document doc = new Document(rootItem); rootItem = doc.getRootElement();
	 * 
	 * subAgentList = getSubAgentList(agentId); // System.out.println(agentId +
	 * "---subAgentList:" // + subAgentList.size());
	 * 
	 * rootItem = setLevelAgentItem(rootItem, rootAgentList, subAgentList);
	 * 
	 * if (subAgentList.size() > 0) { lists.add((ArrayList<Agent>)
	 * subAgentList);
	 * 
	 ...
	 * int levelCount = 10;
	 * 
	 * for (int i = 0; i < levelCount; i++) { currentLevel = i; lists =
	 * setLevelAgentList(subAgentList, lists); rootAgentList = subAgentList;
	 * rootItem = setLevelAgentItem(rootItem, rootAgentList, subAgentList); }
	 * 
	 ...
	 * System.out.println(result); String treeFileName = DateUtil
	 * .getDateString("yyyyMMddHHmmss") + ".xml"; String treeFilePath =
	 * Constant.SERVLET_XML_PATH + treeFileName; XmlUtil_jdom.saveToXmlFile(doc,
	 * treeFilePath); request.setAttribute("relationTreeFileName",
	 * treeFileName);
	 * 
	 * } } catch (Exception e) { e.printStackTrace(); } return request; }
	 * 
	 * protected Element setLevelAgentItem(Element rootItem, ArrayList<Agent>
	 * rootAgentList, ArrayList<Agent> subAgentList) throws AppException {
	 * 
	 * for (int i = 0; i < rootAgentList.size(); i++) { Agent tempRootAgent =
	 * rootAgentList.get(i); long rootAgentId = tempRootAgent.getId(); Agent
	 * tempSupAgent = getSupAgent(tempRootAgent.getId()); Element thisItem =
	 * null; if (tempSupAgent != null) { if (!isExistRoot(tempSupAgent.getId(),
	 * tempRootAgent)) { thisItem = createItem(tempRootAgent); } } else {
	 * thisItem = createItem(tempRootAgent); }
	 * 
	 * // System.out.println(i + "--in the rootAgentList " + // rootAgentName //
	 * + "--subAgentList:" + subAgentList.size()); for (int j = 0; j <
	 * subAgentList.size(); j++) { Agent tempAgent = subAgentList.get(j);
	 * tempSupAgent = getSupAgent(tempAgent.getId()); if (tempSupAgent != null)
	 * { if (rootAgentId == tempSupAgent.getId()) {
	 * 
	 * if (!isExistRoot(tempSupAgent.getId(), tempAgent)) { if (thisItem !=
	 * null) { thisItem.addContent(createItem(tempAgent)); } else {
	 * logger.warn("can not find thisItem "); } }
	 * 
	 * } } }
	 * 
	 * if (thisItem != null) { rootItem.addContent(thisItem); }
	 * 
	 * } return rootItem; }
	 * 
	 * private boolean isExistRoot(long rootAgentId, Agent tempAgent) throws
	 * AppException { boolean flag = false; String itemPath =
	 * "//tree/item/item";
	 * 
	 * if (currentLevel > 0) { for (int i = 0; i < currentLevel; i++) { itemPath
	 * += "/item"; } } else { itemPath = "//tree/item/item"; }
	 * 
	 * //
	 * System.out.println("currentLevel:"+currentLevel+"--itemPath:"+itemPath);
	 * 
	 * List<Element> list = XmlUtil_jdom.getAllElements(rootItem .getDocument(),
	 * itemPath); for (int k = 0; k < list.size(); k++) { Element tempRootItem =
	 * list.get(k); long tempRootAgentId = Constant.toLong(tempRootItem
	 * .getAttributeValue("id"));
	 * 
	 * // System.out.println("rootAgentId:" + rootAgentId // +
	 * "---tempRootAgentId:" + tempRootAgentId); if (tempRootAgentId ==
	 * rootAgentId) { // System.out.println("tempRootAgentId:" +
	 * tempRootAgentId); tempRootItem.addContent(createItem(tempAgent)); flag =
	 * true; break; } } return flag; }
	 * 
	 * private ArrayList<ArrayList<Agent>> setLevelAgentList( ArrayList<Agent>
	 * superAgentList, ArrayList<ArrayList<Agent>> lists) throws AppException {
	 * 
	 * ArrayList<Agent> tempAgentList = new ArrayList<Agent>(); for (int j = 0;
	 * j < superAgentList.size(); j++) { Agent tempAgent =
	 * superAgentList.get(j); ArrayList<Agent> subAgentList =
	 * getSubAgentList(tempAgent .getId());
	 * 
	 * if (subAgentList.size() > 0) { lists.add((ArrayList<Agent>)
	 * subAgentList); tempAgentList.addAll(subAgentList);
	 * 
	 * AgentGroup agentGroup = new AgentGroup(tempAgent, subAgentList);
	 * agentGroupList.add(agentGroup); } } subAgentList = tempAgentList;
	 * 
	 * return lists; }
	 */
