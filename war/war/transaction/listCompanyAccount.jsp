...
<html>
	...
	function add(){
		document.forms[0].companyId.value="<c:out value='${company.id}' ></c:out>";
						...
						<tr>
							...
							<td valign="top" class="body">
								...
								<table width="100%" cellpadding="0" cellspacing="0" border="0" class="dataList">
									...
									<c:forEach var="companyAccount" items="${companyAccountListForm.list}" varStatus="status">
										<tr>
											<td>
												<html:multibox property="selectedItems" value="${companyAccount.id}"></html:multibox>
											</td>
											<td>
												<c:out value="${status.count+(companyAccountListForm.intPage-1)*companyAccountListForm.perPageNum}" />
											</td>
											<td>
												<c:out value="${companyAccount.company.name}" />
											</td>
											<td>
											...
											<td>
												<c:out value="${companyAccount.account.accountAddress}" />
											</td>
											<td>
												<c:out value="${companyAccount.account.tranTypeInfo}" />
											</td>
											<td>
												<c:out value="${companyAccount.account.description}" />
											</td>
											<td>
												<c:out value="${companyAccount.statusInfo}" />
											</td>
										</tr>
									</c:forEach>
								</table>
									...
									<tr>
										<td>
											...
											<c:if test="${!empty company}">
												...
												<input name="label" type="button" class="button1" value="新增账户" onclick="add();">
											</c:if>
										</td>
										<td align="right">
											<div>
												共有记录&nbsp;
												<c:out value="${companyAccountListForm.totalRowCount}"></c:out>
												&nbsp;条&nbsp;&nbsp;&nbsp;&nbsp; [
												<a href="JavaScript:turnToPage(document.forms[0],0)">首页</a> |
												<a href="JavaScript:turnToPage(document.forms[0],1)">上一页</a> |
												<a href="JavaScript:turnToPage(document.forms[0],2)">下一页</a> |
												<a href="JavaScript:turnToPage(document.forms[0],3)"> 末页</a>] 页数:
												<c:out value="${companyAccountListForm.intPage}" />
												/
												<c:out value="${companyAccountListForm.pageCount}" />
												]
											</div>
										</td>
									</tr>
