							...
							<table width="100%" cellpadding="0" cellspacing="0" border="0"
								...
								<tr>
									<td class="lef"><c:out value="${agent.typeInfo}" /> | <c:out
											value="${agent.loyalIndexInfo}" /> | <c:out
											value="${agent.friendIndexInfo}" /> | <c:out
											value="${agent.assetIndexInfo}" /> | <c:out
											value="${agent.specialIndexInfo}" /></td>

								</tr>
								<tr>
									<td class="lef"><c:if test="${!empty agent.directLevel}">
								...
								<tr>
									<td class="lef"><a href="#"
										onclick="showElement('personalBody')">个人信息</a></td>
									<td style="text-align: left" colspan="3"></td>
								</tr>
								<tbody id="personalBody" style="display: none">
									<tr>
										<td class="lef">身份证号</td>
										<td style="text-align: left"><c:out
												value="${agent.cardNo}" /></td>
										<td class="lef">性别/婚姻</td>
										<td style="text-align: left"><c:out
												value="${agent.sexInfo}" /> | <c:out
												value="${agent.marriage}" /></td>
									</tr>
									<tr>
										<td class="lef">生日/年龄</td>
										<td style="text-align: left"><c:out
												value="${agent.birthday}" /></td>
									</tr>
									<tr>
										<td class="lef">关系特征</td>
										<td style="text-align: left"><c:out
												value="${agent.stampTypeInfo}" /></td>
										<td class="lef"></td>
								...
								<tr>
									<td class="lef"><a href="#"
										onclick="showElement('workInfoBody')">简历</a></td>
									<td style="text-align: left" colspan="3"><a href="#"
										onclick="openResume(<c:out value="${agent.id}" />)">详细</a></td>
								</tr>
								<tbody id="workInfoBody">
									<tr>
										<td class="lef">特长</td>
										<td style="text-align: left"><c:out
												value="${agent.strongSuit}" /></td>
										<td class="lef">语言</td>
										<td style="text-align: left"><c:out
												value="${agent.language}" /></td>
									</tr>
								</tbody>
								<tr>
									<td class="lef"><a href="#"
										onclick="showElement('creditInfoBody')">资产负债</a></td>
								...
								<tbody id="creditInfoBody" style="display: none">
									<tr>
										...
										<td style="text-align: left" colspan="3"><c:out
												value="${agent.assetInfo}" /></td>
									</tr>
									<tr>
										<td class="lef">信贷</td>
										<td style="text-align: left" colspan="3">额度: <c:out
												value="${agent.creditAmount}" /> &nbsp; 应收：&nbsp; 应付:
									...
									<c:forEach var="info" items="${creditOrderList}"
										varStatus="status">
										<tr>
											<td class="lef"><c:out value="${status.count}" /></td>
											<td style="text-align: left" colspan="3"><c:out
													value="${info.tranTypeText}" /> <c:if
													test="${!empty info.account}">
													<c:out value="${info.account.showName}" />
												</c:if> 金额： <c:out value="${info.totalAmount}" /> | 手续费： <c:out
													value="${info.handlingCharge}" /> | 发生时间 <c:out
													value="${info.businessDate}" /> | 到期日 <c:out
													value="${info.maturityDate}" /> | <c:out
													value="${info.statusText}" escapeXml="false" /> <a
								...
								<tr>
									<td class="lef"><a href="#"
										onclick="showElement('healthInfoBody')">健康信息</a></td>
									<td style="text-align: left" colspan="3"></td>
								</tr>
								<tbody id="healthInfoBody" style="display: none'">
									<tr>
										<td class="lef">概况</td>
										<td style="text-align: left" colspan="3"><c:out
												value="${agent.healthInfo}" /></td>
									</tr>
								</tbody>
								<tr>
									<td class="lef"><a href="#"
										onclick="showElement('orderInfoBody')">业务明细</a></td>
									<td style="text-align: left" colspan="3">实物金额: <c:out
											value="${agent.physicalAmount}" /> &nbsp; 虚拟金额: <c:out
											value="${agent.virtualAmount}" /> &nbsp; 消费积分: <c:out
											value="${agent.totalIntegral}" />
									</td>
								</tr>
								<tbody id="orderInfoBody" style="display: none">
									<c:forEach var="info" items="${mainOrderList}"
										...
										<tr>
											...
											<td style="text-align: left" colspan="3"><c:out
													value="${info.tranTypeText}" /> <c:if
													test="${!empty info.platform}">
													<c:out value="${info.platform.name}" />
												</c:if> 金额： <c:out value="${info.totalAmount}" /> | 发生时间 <c:out
													value="${info.businessDate}" /> | <c:out
													value="${info.statusText}" escapeXml="false" /> <a
												href="<%=path%>/finance/financeOrderList.do?thisAction=view&id=<c:out value="${info.id}" />">
													查看</a></td>
									</c:forEach>
								...
								<tr>
									...
									<td style="text-align: left"><c:out
											value="${agent.company.name}" /></td>
									<td class="lef">状态</td>
									<td style="text-align: left"><c:out
											value="${agent.statusInfo}" /></td>
								</tr>
							</table>
							<table width="100%" style="margin-top: 5px;">
								<tr>
									<td align="center" colspan="4"><input name="label"
										...
										name="label" type="button" class="button1" value="更新统计"
