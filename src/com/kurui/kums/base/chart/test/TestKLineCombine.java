...
public class TestKLineCombine {
	public static void main(String[] args) {
		...
		try {
			x1Axis.setRange(dateFormat.parse("2007-08-20"), dateFormat
					.parse("2007-09- 29"));// 设置时间范围，注意时间的最大值要比已有的时间最大值要多一天
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		XYPlot plot2 = new XYPlot(timeSeriesCollection, null, y2Axis,
				xyBarRender);// 建立第二个画图区域对象，主要此时的
		// x轴设为了null值，因为要与第一个画图区域对象共享x轴
	...
	// 为了能够保证K线图的上影线和下影线能够和K线图本身的颜色保持一致，找了很多的方法设置都不成功，可以说目前暂不提供单独绘制K线边框颜色的方法，
	// 所以，经过本人的实验，对其中的源代码进行了下修改，具体修改的地方是在下载回来的文件目中
	//source\org\jfree\chart\renderer\xy\CandlestickRenderer.java文件，将
